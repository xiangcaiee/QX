...
if (requestUrl.indexOf('/v1/h5/vip/sub_detail.json') != -1) {
  responseBody = $response.body.replace(/\"old_vip_type\"\:\d+/g, '"old_vip_type":4').replace(/\"is_expire\"\:\d+/g, '"is_expire":0').replace(/\"sub_type\"\:\d+/g, '"sub_type":8').replace(/\"expire_days\"\:.*?/g, '"expire_days":-999').replace(/\"screen_name\"\:\".*?\"/g, '"screen_name":"已破解"').replace(/\"invalid_time\"\:\d+/g, '"invalid_time":1893430861');
  ...
  responseData.data.screen_name = '已破解';
  ...
  responseBody = $response.body.replace(/\"free_trial\"\:\d+/g, '"free_trial":1').replace(/\"vip_type\"\:\d+/g, '"vip_type":1').replace(/\"screen_name\"\:\".*?\"/g, '"screen_name":"已破解"');
